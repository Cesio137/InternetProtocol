...
namespace InternetProtocol {
    ...
    private:
        ...
        void package_string(const std::string &str, const socket_ptr &socket) {
            ...
            while (string_offset < str.size()) {
                ...
                asio::async_write(
                    *socket, asio::buffer(strshrink.data(), strshrink.size()),
                    std::bind(&TCPServer::write, this, asio::placeholders::error,
                              asio::placeholders::bytes_transferred, socket));
        ...
        void read(const asio::error_code &error, const size_t bytes_recvd,
            ...
            if (error) {
                ...
                error_code = error;
                if (!is_closing)
                    disconnect_socket_after_error(error_code, socket);
                return;
            }
    ...
    public:
        ...
        const std::set<ssl_socket_ptr> &get_ssl_sockets() { return tcp.ssl_sockets; }
        ...
        bool open() {
            ...
            tcp.acceptor.listen(backlog > 0 ? backlog : asio::socket_base::max_listen_connections, error_code);
            if (error_code) {
                std::lock_guard<std::mutex> guard(mutex_error);
                on_error(error_code);
                return false;
    ...
    private:
        ...
        void package_string(const std::string &str, const ssl_socket_ptr &ssl_socket) {
            std::lock_guard<std::mutex> guard(mutex_buffer);
            if (!split_buffer || str.size() <= max_send_buffer_size) {
                asio::async_write(*ssl_socket, asio::buffer(str.data(), str.size()),
            ...
            while (string_offset < str.size()) {
                size_t package_size = std::min(max_size, str.size() - string_offset);
                std::string strshrink(str.begin() + string_offset, str.begin() + string_offset + package_size);
                asio::async_write(*ssl_socket, asio::buffer(strshrink.data(), strshrink.size()),
                                  std::bind(&TCPServerSsl::write, this, asio::placeholders::error,
                                            asio::placeholders::bytes_transferred, ssl_socket));
                string_offset += package_size;
            }
        ...
        void package_buffer(const std::vector<uint8_t> &buffer, const ssl_socket_ptr &ssl_socket) {
            std::lock_guard<std::mutex> guard(mutex_buffer);
            if (!split_buffer || buffer.size() <= max_send_buffer_size) {
                asio::async_write(*ssl_socket, asio::buffer(buffer.data(), buffer.size()),
                                  std::bind(&TCPServerSsl::write, this, asio::placeholders::error,
                                            asio::placeholders::bytes_transferred, ssl_socket));
                return;
            }
            ...
            while (buffer_offset < buffer.size()) {
                ...
                std::vector<uint8_t> sbuffer(buffer.begin() + buffer_offset,
                                             buffer.begin() + buffer_offset + package_size);
                asio::async_write(*ssl_socket, asio::buffer(sbuffer.data(), sbuffer.size()),
                                  std::bind(&TCPServerSsl::write, this, asio::placeholders::error,
                                            asio::placeholders::bytes_transferred, ssl_socket));
                buffer_offset += package_size;
            }
        ...
        void consume_response_buffer(const ssl_socket_ptr &socket) {
            if (!listening_buffers.contains(socket)) return;
            const std::shared_ptr<asio::streambuf> streambuf = listening_buffers[socket];
            const size_t size = streambuf->size();
        ...
        void run_context_thread() {
            ...
            tcp.acceptor.async_accept(conn_ssl_socket->lowest_layer(),
                                      std::bind(&TCPServerSsl::accept, this, asio::placeholders::error,
                                                conn_ssl_socket));
            tcp.context.run();
            if (get_acceptor().is_open() && !is_closing) close();
        }
        ...
        void accept(const asio::error_code &error, ssl_socket_ptr &ssl_socket) {
            if (error) {
                ...
                if (tcp.acceptor.is_open()) {
                    ssl_socket_ptr ssl_conn_socket = std::make_shared<asio::ssl::stream<asio::ip::tcp::socket> >(
                        tcp.context, tcp.ssl_context);
            ...
            if (tcp.ssl_sockets.size() <= backlog) {
                ssl_socket->async_handshake(asio::ssl::stream_base::server,
                                            std::bind(&TCPServerSsl::ssl_handshake, this, asio::placeholders::error,
                                                      ssl_socket));
            } else {
                std::lock_guard<std::mutex> guard(mutex_error);
                if (!is_closing)
        ...
        void read(const asio::error_code &error, const size_t bytes_recvd, const ssl_socket_ptr &ssl_socket) {
            ...
            rbuffer.size = bytes_recvd;
            rbuffer.raw_data.resize(bytes_recvd);
            asio::buffer_copy(asio::buffer(rbuffer.raw_data, bytes_recvd), listening_buffers.at(ssl_socket)->data());
